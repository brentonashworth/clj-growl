...
For the example below, my settings look like this:

<img src="http://s3.amazonaws.com/formpluslogic-public/images/clj-growl/growl_settings.png" alt="Growl Settings"/>
...
If you have the <code>growlnotify</code> command installed then you may also send notifications using the <code>growlnotify</code> function.
...
Since the UDP protocol is used, there are no error messages.

If you have the Growl settings window open and are looking at the list of applications, when you send the registration packet, the application will not appear in the list. You have to close the settings window and then re-open it to see your application.

If you have configured a password, you have to use the same password for sending the registration message and the notifications.
