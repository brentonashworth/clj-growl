...
<pre><code>(make-growler [password application]
	          [password application notifications-list])</code></pre>
	                	
Create a function for sending Growl notifications. Optionally register a new application.

<code>password</code> A string which must match the one you enter in the Server Password field in the Growl control panel. If you did not enter a Server password then you may use nil or an empty string here.
...
h3. the growl function returned by make-growler, we will call it "growl"
...
The <code>make-growler</code> function does two things. It registers the application named "MyApp" which configures two types of notifications: "Success" and "Failure" which are both enabled by default. Secondly, it returns a function that may be used to send notifications.
